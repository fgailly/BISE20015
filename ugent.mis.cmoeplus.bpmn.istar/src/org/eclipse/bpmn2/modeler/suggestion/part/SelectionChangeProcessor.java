...
public class SelectionChangeProcessor implements ISelectionListener {
	...
	public void selectionChanged(IWorkbenchPart part, ISelection sel) {
		...
		if (suggestions) {
			if(sugView.getEngine()==null){
				sugView.setEngine(new BPMNSuggestionEngine());
		...
		if (part instanceof EditorPart) {
			sugView.setInput(null);
			ontAdvView.getTableViewer().setInput(null);
		...
		if (suggestions) {
			if ((selection != null) && !(getConstructId().equals("unknown"))) {
				if (!containsPropListener()) {
					System.out.println("Created: " + this.id + " " + this.name + " " + this.type);
	...
	private void addCreateDeletionProcessor() {
		if (!containsModelElementCreationListener()) {
			selection.eAdapters().add(new ModelElementCreationProcessor2());
			System.out.println("deletioncreationlistener added to: " + selection.toString());
		}
	...
	private Boolean castElement() {
		if(selection != null) {
			...
			if (selection instanceof EventImpl) {
				...
				if(pool.basicGetProcessRef() == null){
					setName(pool.getName());
					setUniqueId(pool.getId());
				}
				else{
					setName(pool.basicGetProcessRef().getName());
					setUniqueId(pool.basicGetProcessRef().getId());
					setFather(pool.getName());
				}
