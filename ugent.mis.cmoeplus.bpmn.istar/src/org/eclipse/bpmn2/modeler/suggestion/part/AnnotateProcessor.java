	...
	protected void annotateSuggestion(String id, String label, Recommendation suggestion) {
		if (sel.getType() != "unknown" && sel.getType() != "Error") {
			...
			if (suggestions) {
				...
				IRI modelIRI = ((OntologyView)viewer).getEngine().getManager().addModelInstance(constructIRI, id, label);
				((OntologyView)viewer).getEngine().getManager().addModelAnnotation(modelIRI.toString(), ontologyID, suggestion);
			}
		}	
	}
